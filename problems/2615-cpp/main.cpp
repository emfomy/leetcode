...
// - `0 <= nums[i] <= 10^9`
//
// **Note:** This question is the same as <a> 2121: Intervals Between Identical Elements.</a>
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
class Solution {
 public:
  vector<long long> distance(const vector<int>& nums) {
    ...
    // Loop
    auto ans = vector<long long>(n);
    for (const auto& [_, idxs] : groups) {
      const int l = idxs.size();

      // Prefix sum
      long long prefixSum = 0;
      for (int i = 0; i < l; ++i) {
        long long idx = idxs[i];
        ans[idx] += i * idx - prefixSum;
        prefixSum += idx;
      }
      ...
      for (int i = 0; i < l; ++i) {
        long long idx = idxs[l - i - 1];
        ans[idx] += suffixSum - i * idx;
        suffixSum += idx;
      }
    }
