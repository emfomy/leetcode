// Source: https://leetcode.com/problems/jump-game-v
// Title: Jump Game V
// Difficulty: Hard
// Author: Mu Yang <http://muyang.pro>

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Given an array of integers `arr` and an integer `d`. In one step you can jump from index `i` to index:
//
// - `i + x` where:`i + x < arr.length` and ` 0 <x <= d`.
// - `i - x` where:`i - x >= 0` and ` 0 <x <= d`.
//
// In addition, you can only jump from index `i` to index `j`if `arr[i] > arr[j]` and `arr[i] > arr[k]` for all indices `k` between `i` and `j` (More formally `min(i,j) < k < max(i, j)`).
//
// You can choose any index of the array and start jumping. Return the maximum number of indicesyou can visit.
//
// Notice that you can not jump outside of the array at any time.
//
// **Example 1:**
// https://assets.leetcode.com/uploads/2020/01/23/meta-chart.jpeg
//
// ```
// Input: arr = [6,4,14,6,8,13,9,7,10,6,12], d = 2
// Output: 4
// Explanation: You can start at index 10. You can jump 10 --> 8 --> 6 --> 7 as shown.
// Note that if you start at index 6 you can only jump to index 7. You cannot jump to index 5 because 13 > 9. You cannot jump to index 4 because index 5 is between index 4 and 6 and 13 > 9.
// Similarly You cannot jump from index 3 to index 2 or index 1.
// ```
//
// **Example 2:**
//
// ```
// Input: arr = [3,3,3,3,3], d = 3
// Output: 1
// Explanation: You can start at any index. You always cannot jump to any index.
// ```
//
// **Example 3:**
//
// ```
// Input: arr = [7,6,5,4,3,2,1], d = 1
// Output: 7
// Explanation: Start at index 0. You can visit all the indicies.
// ```
//
// **Constraints:**
//
// - `1 <= arr.length <= 1000`
// - `1 <= arr[i] <= 10^5`
// - `1 <= d <= arr.length`
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

#include <algorithm>
#include <numeric>
#include <vector>

using namespace std;

// DP + Sort
//
// We can only jump downward.
//
// First sort the index by its height.
// We compute the maximum step to reach each index.
//
// We loop from the highest index.
// For each index, update the DP value around it.
class Solution {
 public:
  int maxJumps(const vector<int>& arr, int d) {
    const int n = arr.size();

    // Sort index
    const auto comp = [&arr](int i, int j) -> bool { return arr[i] > arr[j]; };
    auto idxs = vector<int>(n);
    iota(idxs.begin(), idxs.end(), 0);
    sort(idxs.begin(), idxs.end(), comp);

    // Loop
    auto dp = vector<int>(n, 1);
    for (int i : idxs) {
      // Go left
      for (int j = i - 1; j >= 0 && i - j <= d && arr[i] > arr[j]; --j) {
        dp[j] = max(dp[j], dp[i] + 1);
      }

      // Go right
      for (int j = i + 1; j < n && j - i <= d && arr[i] > arr[j]; ++j) {
        dp[j] = max(dp[j], dp[i] + 1);
      }
    }

    return *max_element(dp.cbegin(), dp.cend());
  }
};

// DFS + Cache
//
// We use DFS instead of sorting.
// For each index, we run DFS to find the maximum step from it.
// We cache the result for each index we traversed in the DFS.
class Solution2 {
 public:
  int maxJumps(const vector<int>& arr, int d) {
    const int n = arr.size();

    // Loop
    auto memo = vector<int>(n, 0);
    for (int i = 0; i < n; ++i) {
      dfs(arr, d, i, memo);
    }

    return *max_element(memo.cbegin(), memo.cend());
  }

 private:
  void dfs(const vector<int>& arr, int d, int i, vector<int>& memo) {
    const int n = arr.size();

    if (memo[i] > 0) return;
    memo[i] = 1;

    // Go left
    for (int j = i - 1; j >= 0 && i - j <= d && arr[i] > arr[j]; --j) {
      dfs(arr, d, j, memo);
      memo[i] = max(memo[i], memo[j] + 1);
    }

    // Go right
    for (int j = i + 1; j < n && j - i <= d && arr[i] > arr[j]; ++j) {
      dfs(arr, d, j, memo);
      memo[i] = max(memo[i], memo[j] + 1);
    }
  }
};
