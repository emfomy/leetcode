...
class Solution {
 public:
  ...
  int xSum(vector<int>& nums, int i, int k, int x) {
    constexpr const auto N = 50;
    auto counter = vector<pair<int, int>>(N);  // (count, num)

    // Count
    for (auto num = 1; num <= N; ++num) counter[num - 1].second = num;
    for (auto j = i; j < i + k; ++j) ++counter[nums[j] - 1].first;
    ...
    // Sum
    auto sum = 0;
    for (auto j = 0; j < x; ++j) sum += counter[j].first * counter[j].second;
