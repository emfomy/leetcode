...
class Solution {
 public:
  double maxAverageRatio(vector<vector<int>>& classes, int extraStudents) {
    ...
    // Helper
    auto getDiff = [](vector<int>& cls) -> double {
      double pass = cls[0];
      double total = cls[1];
    ...
    // Prepare data
    auto data = vector<pair<double, int>>();  // diff, idx
    for (auto i = 0; i < n; i++) {
      data.push_back({getDiff(classes[i]), i});
    }

    // Loop
    auto queue = priority_queue(less(), std::move(data));  // max-heap
    for (auto i = 0; i < extraStudents; i++) {
      auto idx = queue.top().second;
      queue.pop();

      auto& cls = classes[idx];
      cls[0]++, cls[1]++;
      queue.push({getDiff(cls), idx});
    }
