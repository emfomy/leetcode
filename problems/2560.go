...
func minCapability(nums []int, k int) int {
	...
	// max house to rob with given capability
	// note that we don't need DP here since we are doing counting
	countHouse := func(cap int) int {
		count := 0
		for i := 0; i < n; i++ {
			if nums[i] <= cap {
				count++
	...
	// Binary Search
	// countHouse(lo-1) < k <= countHouse(lo=hi)
