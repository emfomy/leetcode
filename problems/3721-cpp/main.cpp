...
// Note that since F[i] is contiguous, the zero value must exist (by intermediate value theorem).
//
...
// Otherwise (say previous nums[r] is at i), we only update the range [i+1, r].z
class Solution {
  class SegmentTree {
    // Lazy Tag
    struct Tag {
