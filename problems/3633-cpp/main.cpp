...
// Find the first time that a land ride ends.
// Next find the first time that a water ride ends.
//
// Do the same for water -> land.
class Solution {
 public:
  int earliestFinishTime(                                                  //
      const vector<int>& landStartTime, const vector<int>& landDuration,   //
    ...
    // Find second end
    int secondEnd = INT_MAX;
    for (int i = 0; i < m; ++i) {
      secondEnd = min(secondEnd, max(firstWaterEnd, landStartTime[i]) + landDuration[i]);
    }
    for (int i = 0; i < n; ++i) {
      secondEnd = min(secondEnd, max(firstLandEnd, waterStartTime[i]) + waterDuration[i]);
    }
